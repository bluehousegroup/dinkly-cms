 ...
 */
class CmsAdminGroup extends BaseCmsAdminGroup
{
	...
	public function getPermissions()
	{
		...
		{
			$this->permissions = CmsAdminGroupPermissionCollection::getPermissionsByGroup($this->db, $this->getId());
		}
	...
	public function getMembers()
	{
		...
		{
			$this->members = CmsAdminUserGroupCollection::getUsersByGroup($this->db, $this->getId());
		}
		...
		if($permission_ids != array())
		{
			foreach($permission_ids as $id)
			{
				...
				if(!$permission->isNew())
				{
					...
					$permission_join = new CmsAdminGroupPermission();
					$permission_join->initWithGroupAndPermission($this->getId(), $id);
					
					if($permission_join->isNew())
					{
						$permission_join->setGroupId($this->getId());
						$permission_join->setPermissionId($id);
