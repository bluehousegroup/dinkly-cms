...
class CmsAdminUser extends BaseCmsAdminUser
{
	...
	public function getGroups()
	{
		...
		{
			$this->groups = CmsAdminUserGroupCollection::getGroupsByUser($this->db, $this->getId());
		}
		...
		if($group_ids != array())
		{
			foreach($group_ids as $id)
			{
				...
				if(!$group->isNew())
				{
					...
					if($group_join->isNew())
					{
						$group_join->setUserId($this->getId());
						$group_join->setGroupId($id);
						$group_join->save();
					}
				}
			}
	...
	{
		$link = Dinkly::getConfigValue('current_app_url', 'admin') . "/cms_admin/login/reset_password/k/" . $this->getAutoLoginHash();
	...
	public static function authenticate($username, $input_password)
	{
		...
		$sql = "select * from cms_admin_user where username=".$dbo->quote($username);
