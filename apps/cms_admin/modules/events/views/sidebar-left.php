<div class="sidebar-left">
	<div class="pad active scrollable">
		<div class="add-event">
			<a href="/cms_admin/events/edit" class="btn btn-primary template-submit">Add Event</a>
		</div>
			...
			<?php if ($events != array()): ?>
				<?php foreach ($events as $sidebarEvent): ?>
					<li id="<?= $sidebarEvent->getId() ?>" data-status="<?= ($event && $sidebarEvent->getIsPublished() == 0) ? 'draft' : 'published' ?>" <?= ($event && $sidebarEvent->getId() == $event->getId()) ? 'class="active"' : '' ?>>
						<a href="/cms_admin/events/default/event/<?= $sidebarEvent->getId() ?>"><?= $sidebarEvent->getTitle() ?></a>
					</li>
				<?php endforeach; ?>
			<?php endif; ?>
			<?php if ($events == array()): ?>
